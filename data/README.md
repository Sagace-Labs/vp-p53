# p53 data
    ...
    data/
      p53_tox21.parquet         standardised table (producing the hash)
      example/
        p53_example.parquet     small stratified fixture, used by the tests

`p53_tox21.parquet` holds one row per compound with the two identity columns —
`inchikey` and `smiles` (standardised) — and two label columns. `label` is the
p53 reporter call and `cytotox` is the viability call. Alongside them are
...
signalling pathway, and AID 651633, the cell-viability counter-screen over the
...
The reporter screen decides which compounds the table holds. Compounds the
counter-screen did not call carry a null in `cytotox`, which records that no
call was made.
