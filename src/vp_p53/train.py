...
from vp_p53 import contract as p53_contract
from vp_p53 import data as p53_data
from vp_p53 import model as p53_model
...
def build_version(
    ...
    *,
    ...
    X = fingerprints.featurize(smiles, p53_model.FEATURES)

    fitted = {}
    for output in p53_contract.column_names():
        column = OUTPUT_LABELS[output]
        rows = p53_data.labelled(table, column)
...
def _write_version(
    ...
    *,
    ...
    record = {
        ...
        "supersedes": supersedes,
        "reason": reason,
        "signature": p53_contract.as_manifest_table(),
        "dataset": {
    ...
    print(
        ...
        f"  next: python -m vp_p53.evaluate --version {version}",
        file=sys.stderr,
    )
    return directory
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_p53.train")
