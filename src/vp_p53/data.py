"""The p53 dataset: obtain, standardise, verify.
...
Two endpoints share the table. ``label`` is the reporter call and defines which
compounds the table holds. ``cytotox`` is the viability call from the counter-
screen run on the same library, and is null for a compound that screen did not
call.
...
Run as a command:
    ...
    python -m vp_p53.data fetch --verify
    python -m vp_p53.data verify
"""
...
from vp_p53.target import CYTOTOX, TARGET
...
TABLE_PATH = DATA_DIR / "p53_tox21.parquet"
EXAMPLE_PATH = DATA_DIR / "example" / "p53_example.parquet"
...
def _missing(path: Path) -> str:
    return (
        ...
        "`python -m vp_p53.data fetch` to rebuild it from PubChem."
    )
...
def fetch(*, write: bool = True) -> pd.DataFrame:
    ...
    # A left join: the reporter endpoint decides which compounds the table holds,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m vp_p53.data")
